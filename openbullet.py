...
if __name__ == "__main__":
    ...
    def escolher_arquivo(msg, file_extension=None):
        if file_extension:
        ...
        if not files:
            print(f"{lrd}Nenhum arquivo encontrado no diretório.")
            exit()

        print(f"{gn}Arquivos disponíveis:")
        for i, file in enumerate(files, 1):
            print(f"{i}. {file}")

        escolha = input(f"{msg} (Escolha o número): {cn}")
        try:
            ...
            print(f"{lrd}Escolha inválida! Tente novamente.")
            return escolher_arquivo(msg, file_extension)

    config = escolher_arquivo(f"{lrd}[{lgn}~{lrd}]{gn} ESCOLHA A CONFIG")

    Type = input(f"{lrd}[{lgn}~{lrd}]{gn} TIPO DO PROXY : {cn}")
    try:
